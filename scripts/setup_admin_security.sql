-- Script para configurar segurança administrativa
...
-- 4. Criar políticas de segurança para audit_logs
...
-- 5. Criar políticas para user_roles
...
-- 8. Criar função para verificar se usuário é gerente ou admin
...
-- 9. Atualizar trigger de criação de usuário
...
-- 10. Garantir que o trigger exista
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION handle_new_user();

-- 11. Definir seu usuário como admin (substitua 'rogendor15.com' pelo seu email)
INSERT INTO user_roles (user_id, role)
SELECT id, 'admin'
FROM auth.users
...
-- 12. Atualizar funcionários existentes para ter role apropriado
UPDATE user_roles
SET role = CASE 
  WHEN f.nivel_acesso = 'administrador' THEN 'admin'
  WHEN f.nivel_acesso = 'gerente' THEN 'gerente'
  WHEN f.nivel_acesso = 'recepcionista' THEN 'recepcionista'
  WHEN f.nivel_acesso = 'funcionario' THEN 'funcionario'
  ELSE 'cliente'
END
FROM funcionarios f
...
-- 13. Criar função para admin criar funcionários
...
-- 14. Criar trigger de auditoria para funcionarios
...
-- 15. Adicionar trigger de auditoria na tabela funcionarios
...
SELECT 
  ...
  (SELECT COUNT(*) FROM user_roles WHERE role = 'admin') as admins_count,
